...
def main():
	...
	text_model = CLIPTextModelWithProjection.from_pretrained(settings["clip_text_model_name"]).to(accelerator.device, dtype=torch.float32)
	if accelerator.is_main_process:
	...
	# Handle 
	text_encoder_context = nullcontext()
	last_grad_norm = 0
	# Hopefully this works
	generator.eval()
	with accelerator.accumulate(text_model):
		for e in epoch_bar:
			current_step = 0
			...
			for step, batch in enumerate(dataloader):
				...
				if accelerator.sync_gradients or not accelerator.use_distributed:
					last_grad_norm = accelerator.clip_grad_norm_(text_model.parameters(), 1.0)
